...
import {IUniswapV2Router02} from '../interfaces/pool/IUniswapV2Router02.sol';
 ...
 * @dev Extende UniswapV2Router01 com suporte a tokens com taxas de transferência (fee-on-transfer) e outras melhorias.
 ...
 *      - Suporte a WETH para swaps envolvendo ETH e EIP-2612 para aprovações via assinatura.
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "UniswapV2Router02: EXPIRED_DEADLINE");
        _;
    }
    ...
    constructor(address _factory, address _WETH) {
        require(_factory != address(0), "UniswapV2Router02: INVALID_FACTORY_ADDRESS");
        require(_WETH != address(0), "UniswapV2Router02: INVALID_WETH_ADDRESS");
        factory = _factory;
        WETH = _WETH;
    }
    ...
    receive() external payable {
        require(msg.sender == WETH, "UniswapV2Router02: ETH_FROM_NON_WETH");
    }
    ...
    function _addLiquidity(
        ...
        require(tokenA != address(0) && tokenB != address(0), "UniswapV2Router02: INVALID_TOKEN_ADDRESS");
        require(tokenA != tokenB, "UniswapV2Router02: IDENTICAL_TOKENS");
        require(amountADesired > 0 && amountBDesired > 0, "UniswapV2Router02: INVALID_DESIRED_AMOUNT");
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                ...
                require(amountAOptimal <= amountADesired, "UniswapV2Router02: EXCESSIVE_A_AMOUNT");
    ...
    function _swap(uint256[] memory amounts, address[] memory path, address _to) internal virtual {
        require(path.length >= 2, "UniswapV2Router02: INVALID_PATH_LENGTH");
        require(_to != address(0), "UniswapV2Router02: INVALID_RECIPIENT_ADDRESS");
    ...
    /**
     * @notice Realiza um swap de tokens ao longo de um path, suportando tokens com taxas de transferência.
     * @dev Calcula dinamicamente as quantidades de saída com base nos saldos reais após transferências.
     *      Reverte se o path for inválido ou o swap falhar.
     * @param path Array de endereços de tokens definindo a rota do swap.
     * @param _to Endereço que receberá os tokens de saída.
     */
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        ...
        for (uint256 i; i < path.length - 1; ++i) {
            ...
            uint256 amountOutput;
            {
                ...
                require(amountInput > 0, "UniswapV2Router02: INSUFFICIENT_INPUT_AMOUNT");
    ...
    function addLiquidity(
        ...
        require(to != address(0), "UniswapV2Router02: INVALID_RECIPIENT_ADDRESS");
    ...
    function addLiquidityETH(
        ...
        require(token != address(0), "UniswapV2Router02: INVALID_TOKEN_ADDRESS");
        require(to != address(0), "UniswapV2Router02: INVALID_RECIPIENT_ADDRESS");
        require(msg.value > 0, "UniswapV2Router02: INSUFFICIENT_ETH_VALUE");

        (amountToken, amountETH) = _addLiquidity(token, WETH, amountTokenDesired, msg.value, amountTokenMin, amountETHMin);
        address pair = UniswapV2Library.pairFor(factory, token, WETH);
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWETH(WETH).deposit{value: amountETH}();
        require(IWETH(WETH).transfer(pair, amountETH), "UniswapV2Router02: WETH_TRANSFER_FAILED");
        liquidity = IUniswapV2Pair(pair).mint(to);
    ...
    function removeLiquidity(
        ...
        require(tokenA != address(0) && tokenB != address(0), "UniswapV2Router02: INVALID_TOKEN_ADDRESS");
        require(to != address(0), "UniswapV2Router02: INVALID_RECIPIENT_ADDRESS");
        require(liquidity > 0, "UniswapV2Router02: INVALID_LIQUIDITY_AMOUNT");

        address pair = UniswapV2Library.pairFor(factory, tokenA, tokenB);
        require(IUniswapV2Pair(pair).transferFrom(msg.sender, pair, liquidity), "UniswapV2Router02: TRANSFER_FAILED");
    ...
    /**
     * @notice Remove liquidez de um par token/ETH, suportando tokens com taxas de transferência.
     * @dev Remove liquidez, transfere o saldo total do token e converte WETH em ETH.
     *      Reverte se as quantidades devolvidas forem insuficientes.
     * @param token Endereço do token ERC-20.
     * @param liquidity Quantidade de tokens de liquidez a queimar.
     * @param amountTokenMin Quantidade mínima do token.
    ...
    /**
     * @notice Remove liquidez de um par token/ETH com assinatura EIP-2612, suportando tokens com taxas de transferência.
     * @dev Aprova a transferência via permit e chama removeLiquidityETHSupportingFeeOnTransferTokens.
     *      Reverte se a assinatura for inválida ou as quantidades forem insuficientes.
     * @param token Endereço do token ERC-20.
     * @param liquidity Quantidade de tokens de liquidez a queimar.
     * @param amountTokenMin Quantidade mínima do token.
    ...
    function swapExactTokensForTokens(
        ...
        require(amountIn > 0, "UniswapV2Router02: INVALID_INPUT_AMOUNT");
        require(to != address(0), "UniswapV2Router02: INVALID_RECIPIENT_ADDRESS");
        require(path.length >= 2, "UniswapV2Router02: INVALID_PATH_LENGTH");
    ...
    function swapTokensForExactTokens(
        ...
        require(amountOut > 0, "UniswapV2Router02: INVALID_OUTPUT_AMOUNT");
        require(to != address(0), "UniswapV2Router02: INVALID_RECIPIENT_ADDRESS");
        require(path.length >= 2, "UniswapV2Router02: INVALID_PATH_LENGTH");
    ...
    function swapExactETHForTokens(
        ...
        require(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]), "UniswapV2Router02: WETH_TRANSFER_FAILED");
        _swap(amounts, path, to);
    }
    ...
    /**
     * @notice Realiza um swap de uma quantidade exata de tokens por outra, suportando tokens com taxas de transferência.
     * @dev Transfere tokens de entrada e verifica o saldo final do destinatário.
     *      Reverte se a quantidade de saída for menor que o mínimo especificado.
     * @param amountIn Quantidade de tokens de entrada.
     * @param amountOutMin Quantidade mínima de tokens de saída.
     * @param path Array de endereços de tokens definindo a rota do swap.
    ...
    /**
     * @notice Realiza um swap de ETH por tokens, suportando tokens com taxas de transferência.
     * @dev Converte ETH em WETH, executa o swap e verifica o saldo final do destinatário.
     *      Reverte se o path não começar com WETH ou a saída for insuficiente.
     * @param amountOutMin Quantidade mínima de tokens de saída.
     * @param path Array de endereços de tokens definindo a rota do swap.
     * @param to Endereço que receberá os tokens de saída.
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amountIn), "UniswapV2Router02: WETH_TRANSFER_FAILED");
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    /**
     * @notice Realiza um swap de tokens por ETH, suportando tokens com taxas de transferência.
     * @dev Executa o swap, converte WETH em ETH e transfere para o destinatário.
     *      Reverte se o path não terminar com WETH ou a saída for insuficiente.
     * @param amountIn Quantidade de tokens de entrada.
     * @param amountOutMin Quantidade mínima de ETH de saída.
